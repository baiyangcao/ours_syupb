...
> `Q:` 新的事项也需要坐标导入功能，如何配置导入的图层？
> 
> `A:` 坐标导入的图层需要在数据库中配置，位于 12/13 图形库上的 `arcgis000图形参数配置` 表中
> 使用 `SELECT xml FROM arcgis000图形参数配置 WHERE 参数ID = '60201'`
> 查询出坐标导入配置，在其中添加 `<事项名称/代码>导入图层</事项名称/代码>` 配置
> 需要同时添加以事项名称和事项代码作为元素名的 XML 节点。