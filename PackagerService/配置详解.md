...
| ------- | ------ | ------- |
| groupby | 必填 | 分组字段名称 |
| skipfolders | null | 要跳过生成目录的分组字段值列表，用逗号`,`隔开，如分组字段值分别为“第一组”，“第二组”，“第三组”和“第四组”，`skipfolders="第一组,第二组"`表示不生成“第一组”和“第二组”两个子目录，直接继续处理 |
| ignoregroups | null | 要跳过组的分组字段值列表，用逗号`,`隔开，与`skipfolders`类似，但是不仅仅限于跳过目录的生成，同时`group`的子节点也不会被处理 |
...
> 导出文件名选择顺序：`filenamefield`-->`filename`-->`template`
...
#### `<output type="excel" />`子节点配置说明

子节点中主要用于配置excel模板中的变量、数组、表格等smartmarker的数据源配置
  
SQL语句的连接字符串在`output`节点中`conn`配置，而SQL语句中的参数则由数据源中的每一条记录来提供，可以通过`output`节点中的`parameters`属性来过滤

> 模板及配置详情参见[Excel配置](excel配置.md)

### `type="service"`配置

用于调用服务来获取导出附件的二进制文件数据，下载到本地硬盘中（用于电子政务平台附件导出）；或者是调用远程服务，直接导出文件到本地硬盘中（用于图形shape文件的导出）

#### `<output type="service" />`属性说明

| 属性名称 | 默认值 | 属性说明 |
|:------- | ----------- |:------- |
| sql | 必填 | 获取数据所用的sql语句名称，用SQLGetter.GetSqlByAttr方法获取具体的SQL语句 |
| conn | `conn` | 数据库连接字符串名称，指在Web.config或者App.config文件中配置的连接字符串名称 |
| parameters | `null` | 根据数据源，过滤需要作为sql参数用于`output`子节点下配置的sql语句的字段列表，多个字段用`,`隔开，**一般可以忽略** |
| resulttype | `file` | 服务返回结果类型，可取值为`file`和`value`，`file`表示返回的是二进制的文件数据，根据配置的`filenamefield`直接导出在硬盘上，而`value`表示返回的是值，会在日志中记录返回值 |
| filenamefield | `FileName` | 用于配置服务返回的二进制文件数据保存的文件名字段 |
| serviceparameters | `null` | 配置从数据源中提取的调用服务时的请求参数，多个用`,`隔开；若为空则将数据源中的所有字段取做请求参数；若不想从数据源中提取请求参数则配置`-`即可 |
| hasfolderparameter | `0` | 是否将导出目录作为调用服务时的请求参数，`0`表示否，其他值表示是 |

#### `<output type="service" />`子节点配置说明

子节点配置所要请求的服务地址、方法、参数等，详情见[Service配置](service配置.md)

### `type="draft"`配置

用于配置电子政务平台中文稿类型的文档导出，泛指使用`iWebOffice`插件在Web端显示、编辑、保存的Word文档    

因为相关的文稿文件数据都以二进制数据的形式存储在数据库中，故这里需要配置SQL语句取获取相关数据并使用`iWebOffice`所提供的API来进行二进制文件的读写

#### `<output type="draft" />`属性说明
...
| conn | `conn` | 数据库连接字符串名称，指在Web.config或者App.config文件中配置的连接字符串名称 |
| parameters | `null` | 根据数据源，过滤需要作为sql参数用于`output`子节点下配置的sql语句的字段列表，多个字段用`,`隔开，**一般可以忽略** |
| filenamefield | `FileName` | 导出文稿文件名称字段名 |
| fileobjfield | `FileObj` | 导出文稿文件存储的二进制数据的字段名 |  


> `flow`节点下可以配置`folder`节点添加子目录，也可以配置`group`节点添加分组目录并将`flow`节点中获取的数据源进行分组处理，也可以通过配置`query`节点获取新的数据源，或者直接配置`output`节点导出文件，其中`folder`、`group`和`query`可以配置多层，也可相互嵌套，但叶子节点必须是`output`，否则不会有导出结果